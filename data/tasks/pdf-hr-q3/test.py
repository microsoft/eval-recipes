...
"""
Test script for PDF HR Q3 data analysis task.

Checks if the answer contains the correct median salary ($104,700) within 1% tolerance.
...
GROUNDTRUTH_VALUE = 104700.0
TOLERANCE_PERCENT = 1.0


def parse_numeric_value(text: str) -> float | None:
    """
    Parse a numeric value from text, handling various USD formats.
    ...
    cleaned = re.sub(r"[USD$\s]", "", text, flags=re.IGNORECASE)
...
"""
Sample command(s):
uv run data/tasks/pdf-hr-q3/test.py --answer "$104,700" --test-id "test" --output-dir /tmp
uv run data/tasks/pdf-hr-q3/test.py --answer "104700" --test-id "test" --output-dir /tmp
"""
