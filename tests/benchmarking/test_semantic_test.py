...
6. Verify that functions have proper docstrings"""
    ...
    result = await semantic_test(steps=steps, rubric=rubric, context=context, working_dir=project_dir)

    # Validate result is SemanticTestResult
    assert isinstance(result, SemanticTestResult)
